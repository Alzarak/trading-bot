...
**Domain:** Automated stock day trading bot as a Claude Code plugin
**Researched:** 2026-03-21
**Confidence:** HIGH (core trading stack), MEDIUM (plugin system details)
...
pip install alpaca-py>=0.43.2 pandas-ta>=0.4.0b0 pandas>=2.0 numpy>=1.26 pydantic-settings>=2.0 APScheduler>=3.10 loguru>=0.7 rich>=13.0
...
Plugin hook strategy for dependency management:
    ...
    "SessionStart": [
      {
        "hooks": [
          {
            "type": "command",
            "command": "diff -q \"${CLAUDE_PLUGIN_ROOT}/requirements.txt\" \"${CLAUDE_PLUGIN_DATA}/requirements.txt\" >/dev/null 2>&1 || (cd \"${CLAUDE_PLUGIN_DATA}\" && python3 -m pip install -r \"${CLAUDE_PLUGIN_ROOT}/requirements.txt\" -q && cp \"${CLAUDE_PLUGIN_ROOT}/requirements.txt\" .) || rm -f \"${CLAUDE_PLUGIN_DATA}/requirements.txt\""
          }
        ]
      }
    ]
...
**If running within Claude Code (interactive mode):**
...
**If running as standalone Python on a server:**
...
**If paper trading (recommended for all new users):**
...
hist = StockHistoricalDataClient(api_key, secret_key)
```
...
├── commands/
│   ├── initialize.md            # /initialize — interactive setup wizard
│   ├── build.md                 # /build — generate Python scripts from config
│   └── run.md                   # /run — start autonomous trading loop
├── agents/
│   ├── market-analyst.md        # Analyzes market data, scores trade opportunities
│   ├── risk-manager.md          # Evaluates position size, enforces limits
│   └── trade-executor.md        # Places orders, monitors fills
├── skills/
│   ├── trading-context/
│   │   └── SKILL.md             # Auto-loads: user's strategy, risk tolerance, budget
│   ├── alpaca-api-reference/
│   │   └── SKILL.md             # Auto-loads: alpaca-py usage patterns
│   └── risk-rules/
│       └── SKILL.md             # Auto-loads: circuit breaker rules
├── hooks/
│   └── hooks.json               # SessionStart: install Python deps
├── scripts/
│   ├── install_deps.sh          # Called by SessionStart hook
│   └── templates/               # Jinja2 templates for /build-generated scripts
├── .mcp.json                    # Alpaca MCP server config
├── requirements.txt             # Pinned Python deps for trading scripts
└── CHANGELOG.md
```
...
*Stack research for: automated stock day trading Claude Code plugin using Alpaca API*
*Researched: 2026-03-21*
